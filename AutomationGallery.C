...
void duzenle(int c)
{
	...
	if((galeri=fopen("aracbilgileri.txt","r"))!=0)
	{
		while(!feof(galeri))
		{
			fscanf(galeri,"%d\t%s\t%s\t%s\t%s\t%f\t%f\t%f\t\n",(sira+i),(ma+20*i),(se+20*i),(mo+20*i),(re+20*i),(yil+i),(km+i),(fiyat+i)); // eski bilgileri yazýyor burasý 
	...
	*(fiyat+i)=atof(y+40);
	...
	if((galeri=fopen("aracbilgileri.txt","w"))!=0)
	{
		...
		fflush(galeri); // yeni bilgilei eski biilgilerin üstüne yazýyorum; fflush sayesinde de esk
		fclose(galeri);
